...
По умолчанию, все таски отключены, но 11 числа подключу я постоянную основу.
Если вдруг все таски disabled будет true (в tasks/list/), то запускать в таком порядке:
1. do_proxy_processes. Подождать ~5 минут, для получения и запуска валидации прокси. Вообще это довольно долгий процесс, но 5 минут должно хватить.
2. do_parse_data_from_aliexpress. Ему необходимы прокси, которые будут получены из метода, запущенного ранее.
?. do_parse_data_from_chitai_gorod - нет никаких ограничений или наставлений, можно запускать когда угодно.
